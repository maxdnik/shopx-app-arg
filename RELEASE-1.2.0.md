...
Desplegar primero la actualización de API de ShopX que incorpora `POST /api/app/cart/preview`, el cálculo compartido con checkout y la selección semanal en `/api/app/home-sections`. Esta app bloquea el pago si no puede confirmar el importe; no sustituye una falla del servidor por un precio local.

## Verificación realizada

- TypeScript: sin errores.
- Cuatro pruebas de regresión de app: conservación de cargos, carrito concurrente y atómico, identidad de variantes y conversión de cotizaciones.
...
1. Desplegar la API compatible y comprobar el resumen con un producto del catálogo y una cotización externa.
...
4. Probar el build firmado en iPhone y Android: inicio de sesión Google/Apple según plataforma, CUIT y dirección, catálogo y variantes, agregar/quitar cantidades, resumen de precios, cotizaciones manuales y agrupadas, zoom y cierre de fotos, seguimiento parcial, retorno desde Mercado Pago y actualización del estado. Usar cuentas y pagos de prueba autorizados.
...
No se ejecutaron pagos, envíos de cotizaciones, fusiones de cotizaciones ni cambios en pedidos reales durante las pruebas. No se generó ni se envió un build firmado: EAS no tiene sesión iniciada en el entorno de preparación.
